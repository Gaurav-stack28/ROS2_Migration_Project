...
namespace SickToolbox {
  ...
  sick_lms_2xx_scan_resolution_t SickLMS2xx::DoubleToSickScanResolution( const double scan_resolution_double ) {
    return IntToSickScanResolution((static_cast<int>(scan_resolution_double * 100)));
  }
  ...
  void SickLMS2xx::_setTerminalBaud( const sick_lms_2xx_baud_t baud_rate )  {
    ...
    catch(const SickIOException &sick_io_exception) {
      std::cerr << sick_io_exception.what() << std::endl;
      throw;
    }
    
    /* Catch an IO exception */
    catch(const SickThreadException &sick_thread_exception) {
      std::cerr << sick_thread_exception.what() << std::endl;
      throw;
    }
