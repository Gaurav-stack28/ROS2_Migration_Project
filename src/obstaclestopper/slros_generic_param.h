...
/**
 * ROS 2 parameter parser
 *
 * ROS 1 used XmlRpc::XmlRpcValue.
 * ROS 2 parameters are strongly typed, so direct assignment is sufficient.
 */
template <class T>
bool getScalar(const T& value, T& paramValue)
{
    paramValue = value;
    return true;
}
...
uint8_t SimulinkParameterGetter<CppParamType,ROSCppParamType>::get_parameter(CppParamType* dataPtr)
{
    ...
    if (nodePtr->get_parameter(paramName, paramValue))
    {
    paramRetrieved = true;
    }    
...
void SimulinkParameterSetter<CppParamType,ROSCppParamType>::set_parameter(const CppParamType& value)
{
    ...
    nodePtr->set_parameter(rclcpp::Parameter(paramName, paramValue));
}
