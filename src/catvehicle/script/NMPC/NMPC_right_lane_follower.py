...
class LaneFollower(Node):
    def _log_throttle(self, level, key, period, message):
        ...
        last = getattr(self, '_log_times', {}).get(key, -float('inf'))
        if now - last >= period:
            if not hasattr(self, '_log_times'):
                self._log_times = {}
    ...
    def __init__(self, init_node=True):
        ...
        self.cmd_vel_pub = self.create_publisher( Twist, '/catvehicle/cmd_vel', 100 )
        ...
        # Odom subscriber 
        self.odom_sub = self.create_subscription( Odometry, '/catvehicle/odom', self.odom_callback, 10 )
        # "============================================"
        # for EMA
    ...
    def point_cloud_callback(self, msg):
        point_list = []
        for point in pc2.read_points( msg, field_names=("x", "y", "z", "rgba"), skip_nans=True ):
            x, y, z, rgba = point
            rgba = int(rgba)
            r = int((rgba >> 16) & 0xFF)
            g = int((rgba >> 8) & 0xFF)
        ...
        self.get_logger().info(f"Filtered {len(point_list)} white points")
        if len(point_list) == 0:
    ...
    def process_point_list(self):
        ...
        if points.size == 0:
            self.get_logger().info("No white points found")
            self.lane_detected = False
            return
        points_2d = points[:, :2]
        ...
        self.get_logger().info(f"Number of clusters found: {len(set(labels)) - (1 if -1 in labels else 0)}")
        self.get_logger().info(f"delta_interceptSCAN parameters - eps: {eps_value}, min_samples: {min_samples_value}")
        ...
        if len(lanes) >= 1:
            ...
            self.get_logger().info("Expected at least 1 lane, but found none.")
            self.lane_detected = False
    ...
    def move_vehicle_ppc(self):     # PURE PURSUIT CONTROL
        ...
        if look_ahead_y < -5 or look_ahead_y > 5:
            self.get_logger().info("Invalid look-ahead point, stopping vehicle")
            twist = Twist()
            twist.linear.x = 0
            twist.angular.z = 0
            self.cmd_vel_pub.publish(twist)
    ...
    def update_mpc_visualization(self):
        ...
        if hasattr(self, 'bl_lines_odom') and len(self.bl_lines_odom) > 0:
            ...
            for idx, line_points in enumerate(self.bl_lines_odom):
                ...
                self.ax.plot(line_points[:, 0], line_points[:, 1], color=color, linestyle='-', linewidth=1.0, label=label)
        # 2.Plot MPC reference trajectory
        if self.last_ref_traj is not None:
            self.ax.plot(self.last_ref_traj[:, 0], self.last_ref_traj[:, 1], 'r-', linewidth=1.0, marker=' ', markersize=2, markevery=2, label='MPC Reference Path')
        # 3.Plot vehicle's actual traveled path
        if len(self.vehicle_path) > 2:
            path_x, path_y = zip(*self.vehicle_path)
            self.ax.plot(path_x, path_y, 'b-', linewidth=1.2, alpha=0.7, label='Actual Vehicle Path')
        # 4.Plot current vehicle position
        self.ax.plot(x_vehicle, y_vehicle, 'ro', markersize=5, label='Vehicle Position', zorder=5)
        # Vehicle heading arrow
        arrow_length = 0.5
        dx_arrow = arrow_length * np.cos(psi_vehicle)
        dy_arrow = arrow_length * np.sin(psi_vehicle)
        self.ax.arrow(x_vehicle, y_vehicle, dx_arrow, dy_arrow, head_width=0.5, head_length=0.3, fc='red', ec='red', linewidth=2, zorder=5)
    ...
    def odom_callback(self, msg):
        ...
        self._log_throttle( 'info', 'odom_state', 1.0, f"[ODOM_STATE] x={x:+.2f} | y={y:+.2f} | " f"yaw={yaw:+.2f} | v={self.current_speed:+.2f}" )
    def move_vehicle_mpc(self):
        # Safety: lane visibility
        if not self.lane_markings_visible:
            self.get_logger().warning("MPC: no lane markings -> STOP")
            twist = Twist()
            twist.linear.x = 0.0
            twist.angular.z = 0.0
            self.cmd_vel_pub.publish(twist)
        ...
        if not self.lane_detected or self.target_path is None:
            if len(self.lane_history) > 0:
                self.target_path = self.calculate_target_path(self.fit_boundary_lines(self.lane_history[-1]))
                self.get_logger().warning("MPC: using lane history fallback")
            if self.target_path is None:
                self.get_logger().warning("MPC: no target path -> STOP")
                twist = Twist()
                twist.linear.x = 0.0
                twist.angular.z = 0.0
                self.cmd_vel_pub.publish(twist)
        ...
        if not getattr(self, "odom_initialized", False):
            self.get_logger().warning( "MPC: waiting for /catvehicle/odom: STOP")
            twist = Twist()
            twist.linear.x = 0.0
            twist.angular.z = 0.0
            self.cmd_vel_pub.publish(twist)
        ...
        if center is None:
            self.get_logger().warning( "MPC: centerline not available: STOP")
            twist = Twist()
            twist.linear.x = 0.0
            twist.angular.z = 0.0
            self.cmd_vel_pub.publish(twist)
        ...
        try:
            steering_rad = self.mpc_controller.compute_controls( current_state=self.current_state, reference_traj=ref_traj, v_ref=vref )
        except Exception as e:
            self.get_logger().error(f"MPC solve failed: {str(e)}")
            return
        t_end = self.get_clock().now().nanoseconds / 1e9
        self.get_logger().info(f"MPC_TIME_MS={(time.perf_counter()-t0)*1000:.1f}")
        # Apply sign
        ...
        self.get_logger().info(f"Total MPC Pipeline: {_t_pipeline_ms:.2f} ms")
        # Visualization
        if self.use_mpc:
            self.update_mpc_visualization()
        if len(self.vehicle_path) > 15:
            self.vehicle_path.pop(0)
        self.get_logger().info( f"[MPC_ODOM] mC={mC:+.3f} bC={bC:+.3f} | " f"psi0={psi0:+.3f} psi_ref={ref_traj[0,2]:+.3f} | steer={steering_rad:+.3f}" )
        self.get_logger().info(f"MPC_TIME_MS={(t_end - t_start)*1000:.2f}")
    def run(self):
        self.mpc_controller.reset_warmstart()
        try:
            while rclpy.ok():
                # Process ROS 2 callbacks for point cloud and odometry.
