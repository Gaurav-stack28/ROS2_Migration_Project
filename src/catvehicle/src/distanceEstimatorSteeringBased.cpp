...
std_msgs::msg::Float64 Xdist;
std_msgs::msg::Float64 Ydist;
...
rclcpp::Time lastUpdate, currentUpdateTime;

std::vector<std_msgs::msg::Float64> distVector;
std::vector<std_msgs::msg::Float64> velVector;
std::vector<double> timeVector;

rclcpp::Duration deltaT = rclcpp::Duration::from_seconds(0.0);
bool newMessage;
...
int reportCount = 0;
void steeringCallback( const geometry_msgs::msg::Wrench::ConstSharedPtr steering)
{
...
int main( int argc, char **argv )
{
    ...
    auto node = std::make_shared<rclcpp::Node>("DistanceSteeringBased");
    ...
    node->declare_parameter<std::string>("Xdist_topic", "Xdist");
    Xdist_topic = node->get_parameter("Xdist_topic").as_string();
    node->declare_parameter<std::string>("Ydist_topic", "Ydist");
    Ydist_topic = node->get_parameter("Ydist_topic").as_string();
...
//    n.param("vel_topic", vel_topic, std::string("vel"));
    node->declare_parameter<std::string>("scan_topic", "/catvehicle/front_laser_points");
    scan_topic = node->get_parameter("scan_topic").as_string();
    node->declare_parameter<std::string>("scan_topic", "/catvehicle/front_laser_points");
    steering_topic = node->get_parameter("steering_topic").as_string();
    ...
    node->declare_parameter<double>("tmin", -3.0);
    tmin = node->get_parameter("tmin").as_double();
    node->declare_parameter<double>("tmax", 3.0);
    tmax = node->get_parameter("tmax").as_double();
    node->declare_parameter<double>("smin", 0.0);
    smin = node->get_parameter("smin").as_double();
    node->declare_parameter<double>("smax", 80.0);
    smax = node->get_parameter("smax").as_double();
    node->declare_parameter<double>("epsilon", 0.005);
    ...
    auto Xdist_pub = node->create_publisher<std_msgs::msg::Float64>(Xdist_topic, 1);
    auto Ydist_pub = node->create_publisher<std_msgs::msg::Float64>(Ydist_topic, 1);
  ...
  //  ros::Publisher vel_pub = n.advertise<std_msgs::Float64>(vel_topic, 1);
    ...
    auto substeering = node->create_subscription<geometry_msgs::msg::Wrench>(steering_topic,1,steeringCallback);

    RCLCPP_INFO_STREAM(node->get_logger(),"Looking for scan in topic " << scan_topic);
    RCLCPP_INFO_STREAM(node->get_logger(),"Looking for steering in topic " << steering_topic);
	RCLCPP_INFO_STREAM(node->get_logger(),"Steering fudge factor = " << SteeringAngleFudgeFactor);
	...
	// Corner case checking:
	{
		...
		RCLCPP_INFO_STREAM(rclcpp::get_logger("DistanceSteeringBased"),"Consider a point at (" << x << "," << y << ") in front of the laser:");
		RCLCPP_INFO_STREAM(rclcpp::get_logger("DistanceSteeringBased"),"Laser Mag = " << laserMag << "\tangle=" << laserAngle);
		RCLCPP_INFO_STREAM(rclcpp::get_logger("DistanceSteeringBased"),"Straight: t = " << x << "\ts=" << y);
		RCLCPP_INFO_STREAM(rclcpp::get_logger("DistanceSteeringBased"),"Right:    t = " << mRingCoordinateRight.t<< "\ts=" << mRingCoordinateRight.s);
		RCLCPP_INFO_STREAM(rclcpp::get_logger("DistanceSteeringBased"),"Left:     t = " << mRingCoordinateLeft.t<< "\ts=" << mRingCoordinateLeft.s);
		double errorLeft = fabs(y - mRingCoordinateLeft.s);
		double errorRight = fabs(y - mRingCoordinateRight.s);
		RCLCPP_INFO_STREAM(rclcpp::get_logger("DistanceSteeringBased"),"Expeceted error based on Epsilon: "<< (errorLeft > errorRight ? errorLeft : errorRight));
	}
