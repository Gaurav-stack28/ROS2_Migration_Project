...
namespace gazebo
{
    class distance: public SensorPlugin
    {
        private:
            ...
            rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr distance_publisher; // ROS publisher for minimum distance information (lead object)
            rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr relvel_publisher; // ROS publisher for relative velocity of the lead object
            rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr angle_publisher; // ROS publisher for angle of minimum distance information
            rclcpp::Node::SharedPtr rosnode; // ROS Node
            sensor_msgs::msg::LaserScan laser_data; // stores received laser scan data
            std_msgs::msg::Float64 minimum_distance; // saves minimum distance found
            std_msgs::msg::Float64 angle_of_min_distance; //saved the angle of minimum distance
            geometry_msgs::msg::Twist relvel_data;
            double angle_min;
            double angle_max;
        ...
        public:
            ...
            distance()
            {
                ...
                lastUpdate = rclcpp::Time(0, 0, RCL_ROS_TIME);
                rv_length = 0;
            ...
            void OnNewScan(ConstLaserScanStampedPtr &_msg)
            {
                ...
                if(isinf(min_dist))
                {
                    RCLCPP_ERROR_STREAM(rclcpp::get_logger("distance"),"Infinity Distance Detected. Setting to zero.");
                }
                this->minimum_distance.data = min_dist;
                this->angle_of_min_distance.data = min_angle;

                rclcpp::Time nowtime = rclcpp::Clock(RCL_ROS_TIME).now();
                rclcpp::Duration diff = nowtime - lastUpdate;
                double elapsedTime = diff.seconds();
                double nanoSecs = diff.nanoseconds()*1e-9;
              ...
              //  ROS_INFO_STREAM("nanoSec: " << nanoSecs);
                ...
                if (oldtimedata != nowtime.seconds())
                {
                    ...
                    old_time.at(rv_length) = nowtime.seconds();
                }
                ...
                {
                    RCLCPP_ERROR_STREAM(rclcpp::get_logger("distance"),"Estimated (filtered) relative distance is set at infinity.");
                }
            ...
            {
                RCLCPP_INFO_STREAM(rclcpp::get_logger("distance"),"Minimum distance estimator SensorPlugin Loaded.");
                ...
                this->parent_sensor = std::dynamic_pointer_cast<sensors::RaySensor>(parent);
                ...
                RCLCPP_INFO_STREAM(rclcpp::get_logger("distance"),"[distance SensorPlugin] Namespace retrieved is: " << this->ns);
                this->distance_topic = this->ns + "/lead_dist";
                this->angle_topic = this->ns + "/lead_angle";
                this->relvel_topic = this->ns + "/rel_vel";
                ...
                RCLCPP_INFO_STREAM(rclcpp::get_logger("distance"),"[distance SensorPlugin] Plugin's parent name: "<< this->parent_sensor->Name());
                RCLCPP_INFO_STREAM(rclcpp::get_logger("distance"),"[distance SensorPlugin] Plugin's granparent: "<< this->parent_sensor->ParentName());
                RCLCPP_INFO_STREAM(rclcpp::get_logger("distance"),"[distance SensorPlugin] Plugin's full scope name: "<< this->parent_sensor->ScopedName());
                this->rosnode = std::make_shared<rclcpp::Node>(this->ns + "/distance");
                this->distance_publisher = this->rosnode->create_publisher<std_msgs::msg::Float64>(this->distance_topic, 1);
                this->angle_publisher = this->rosnode->create_publisher<std_msgs::msg::Float64>(this->angle_topic, 1);
                this->relvel_publisher = this->rosnode->create_publisher<geometry_msgs::msg::Twist>(this->relvel_topic, 1);
                ...
                RCLCPP_INFO_STREAM(rclcpp::get_logger("distance"),"SENSOR PLUGIN LOADED");
            }
    };
